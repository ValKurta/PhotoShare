...
class Auth:
    ...
    async def create_token(
        ...
        to_encode.update(
            {"iat": datetime.now(timezone.utc), "exp": expire, "scope": scope}
        )
    ...
    async def create_access_token(
        ...
        return await self.create_token(
            data, expires_delta, "access_token", timedelta(minutes=30)
        )
    ...
    async def create_refresh_token(
        ...
        return await self.create_token(
            data, expires_delta, "refresh_token", timedelta(days=7)
        )

    async def decode_refresh_token(self, refresh_token: str) -> str:
        try:
            payload = jwt.decode(
                refresh_token, self.JWT_SECRET_KEY, algorithms=[self.ALGORITHM]
            )
