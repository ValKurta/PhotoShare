...
async def filter_photos(
    ...
    try:
        ...
        start_date_parsed = date_adapter.validate_python(start_date) if start_date else None
    ...
    photos = filter_photos_by_criteria(db, min_rating, max_rating, start_date_parsed, end_date_parsed)

    if not photos:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Photo(s) not found!")

    result = []
    for photo in photos:
        avg_rating = db.query(func.avg(Rating.rating)).filter(Rating.photo_id == photo.id).scalar() or 0
        result.append({
            "id": photo.id,
            "url": photo.url,
