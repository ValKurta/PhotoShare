...
async def create_photo(text: str, url: str, user: User, db: Session) -> Photo:
    ...
    photo = Photo(user_id=user.id, url=url, description=text)
    db.add(photo)
    db.commit()
    db.refresh(photo)
    return photo
...
async def update_photo(
    photo_id: int, url: UploadFile, description: str, user: User, db: Session
) -> Photo | None:
...
async def get_users_photos(
    user_id: int, user: User, db: Session
) -> List[Type[Photo]] | None:
    user = (
        db.query(User)
        .join(Photo)
        .filter(and_(User.id == user_id, Photo.user_id == user.id))
        .first()
    )
    if user is None:
        raise HTTPException(
